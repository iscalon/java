...
class NeuralNetworkTraining implements NeuralNetworkTrainUseCase {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(NeuralNetworkTraining.class);
  ...
  private static final double EXPECTED_ERROR_RATE = 0.000003;
  private static final double DELTA = 0.0000001;
  private static final int MAXIMUM_TRAIN_LOOP_COUNT = 5000;

  private final XYGraphicHandler graphicHandlers;
  private final DataRepository<BasicNetwork> networks;

  NeuralNetworkTraining(XYGraphicHandler graphicHandlers, DataRepository<BasicNetwork> networks) {
  ...
  private static boolean train(BasicNetwork network, MLDataSet dataSet) {
    ...
    do {
      ...
      if (epoch >= MAXIMUM_TRAIN_LOOP_COUNT
        ...
        LOGGER.warn(
            "L'entrainement du réseau a échoué, peut-être devriez vous relancer l'entrainement ?");
        return false;
      }
  ...
  private void computeGraphics(
    ...
    for (MLDataPair actualInputAndOutput : dataSet) {
      ...
      double xNomalizedValue = inputData.getData(0);
      double yNormalizedValue = outputData.getData(0);
      double yNormalizedPredictedValue = predictedOutput.getData(0);

      double xDenormalizedValue = xProjection.normalize(xNomalizedValue).getFirst();
